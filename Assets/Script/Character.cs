...
public class Character : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        PrintGraph(graph.GetCurrentNode(), 0, listConditions);
    }
    ...
    private void PrintGraph(Libs.Graph.GraphNode _node, System.UInt32 _depth, List<Edge.Condition> _conditions)
    {
        Libs.Graph.GraphNode currentNode = _node;
        System.UInt32 depth = _depth;
        string space = "";
        while((depth--)>0)
        {
            space += " ";
        }
        Debug.Log(space+currentNode.ToString());
        foreach (Edge.Condition c in _conditions)
        {
            Libs.Graph.GraphNode transition = currentNode.Transition(c);
            if (transition != currentNode)
            {
                PrintGraph(transition, _depth+1, _conditions);
            }
        }
    }
}
